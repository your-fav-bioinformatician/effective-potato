...
class UniBridgeApi extends ChangeNotifier {
  ...
  Future<Map<String, dynamic>?> getUserProfile() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/user/profile/$_currentUserId'),
        headers: _headers,
      );
  ...
  Future<List<dynamic>> getCatalogMajors({String? search}) async {
    try {
      final url = search != null && search.isNotEmpty 
          ? '$baseUrl/catalog/majors?search=$search' 
          : '$baseUrl/catalog/majors';
  ...
  Future<List<dynamic>> getBookmarks() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/user/bookmarks/$_currentUserId'),
        headers: _headers,
      );
      if (response.statusCode == 200) return jsonDecode(response.body);
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> getMatchBreakdown(String majorId) async {
     if (_currentUserId == null) return null;
     try {
       final response = await http.get(
         Uri.parse('$baseUrl/results/breakdown/$_currentUserId/$majorId'),
         headers: _headers,
       );
       if (response.statusCode == 200) return jsonDecode(response.body);
     } catch (e) {
